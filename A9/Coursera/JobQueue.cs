...
class ParallelProcessing
{
    ...
    {
        var tokens = Console.ReadLine().Split().Select(s => long.Parse(s)).ToArray();
        long nWorkers = tokens[0];
        long nJobs = tokens[1];
        
        long[] jobs = new long[nJobs];
        tokens = Console.ReadLine().Split().Select(s => long.Parse(s)).ToArray();
        for (long i = 0; i < nJobs; i++)
        {
            jobs[i] = tokens[i];
        }

        var assignedJob = AssignJobs(nWorkers,jobs);
        foreach (var job in assignedJob)
        {
            Console.WriteLine(job.worker + " " + job.startedTime);
        }
        Console.ReadKey();
    }
}